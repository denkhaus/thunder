...
//
// Two types are compatible if they are the same besides
// non-nullable modifiers.
//
// The merged type gets non-nullable modifiers depending on how
// the type is used.
// For input types, the merged type should be accepted by both
// services, so it's nullable only if both services accept a
// nullable type.
// For output types, the merged type should work for either
// service, so it's nullable if either service might return null.
